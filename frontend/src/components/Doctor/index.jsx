...
const DoctorList = () => {
  ...
  return (
    <div className="mx-5 py-7">
      <h1 className="text-center text-4xl text-indigo-600 font-extrabold mb-10">Doctor Directory</h1>
      <div className="flex flex-col md:flex-row gap-6">
        {/* Sidebar Filters */}
        <div className="w-full md:w-1/4">
          <div className="bg-white p-5 rounded-lg shadow-lg border border-gray-300">
            <h5 className="text-xl font-bold mb-4 text-indigo-700">Specialization</h5>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentDoctors.map((doctor) => (
              ...
              >
                ...
                />
                <h5 className="text-xl font-bold text-center text-indigo-700">{doctor.name}</h5>
                <p className="text-gray-600 text-center text-lg">{doctor.specialization}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex justify-center mt-8 space-x-3">
        <button
          className="px-5 py-2 rounded-full text-lg font-semibold transition-all bg-gray-200 hover:bg-gray-300 disabled:opacity-50"
          onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))}
          disabled={currentPage === 1}
        >
          ◀ Prev
        </button>
        <span className="px-6 py-2 rounded-full bg-indigo-500 text-white text-lg font-semibold">Page {currentPage} of {totalPages}</span>
        <button
          className="px-5 py-2 rounded-full text-lg font-semibold transition-all bg-gray-200 hover:bg-gray-300 disabled:opacity-50"
